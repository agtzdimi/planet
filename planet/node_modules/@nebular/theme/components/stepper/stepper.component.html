<ng-template>
  <ng-content select="nb-step"></ng-content>
</ng-template>
<div class="header">
  <ng-container *ngFor="let step of steps; let index = index; let first = first">
    ...
    <div *ngIf="!step.hidden" class="step" [class.selected]="isStepSelected(step)"
      [class.completed]="!isStepSelected(step) && step.completed" [class.noninteractive]="disableStepNavigation"
      (click)="!disableStepNavigation && step.select()">
      <div class="label-index">
        <span *ngIf="!step.completed || isStepSelected(step)">{{ index + 1 }}</span>
        <i *ngIf="!isStepSelected(step) && step.completed" class="check outline icon"></i>
      </div>
      <div class="label">
