...
class SimulationData (threading.Thread):
   ...
   def run(self):
      proc = subprocess.Popen(['mosquitto_sub','-h','160.40.52.193','-t','simulations_results'],stdout=subprocess.PIPE)
...
class BarStatus (threading.Thread):
   ...
   def run(self):
      proc = subprocess.Popen(['mosquitto_sub','-h','160.40.52.193','-t','simulations_status'],stdout=subprocess.PIPE)
      for msg in iter(proc.stdout.readline,''):
         msg = msg.decode("utf-8")
         try:
