...
if [[ "$fileName" == "Results2" ]]; then
   ...
   if (( linesF2 == 1 )) && [[ -s "$dirName/${fileName}.csv" ]]; then
      ...
      mongoexport --port $MONGO_PORT --host $MONGO_IP -u $MONGO_USER -p $MONGO_PASSWORD \
       --authenticationDatabase $MONGO_AUTH_DB --db planet --collection results -q '{"formName": "'"$formName"'"}' --type=csv --fields Data_names,Grafico_tot,formName --out "$dirName/${fileName}.csv"
